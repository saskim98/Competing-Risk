	...
	integer, parameter :: nx = 2, np = 5, nl = 3
      integer, parameter ::  initial = 2000, initer = 100000
	...
	real*8 lambda(ni), zeta(ni), ezeta, api
	...
	real*8 bardic, dicbar, dic
      ...
      external gen_dic
    	...
    	open(unit = 6, file = 'LaborFN1_Initial.txt')
    	    	
    	open(unit = 11, file = 'LaborFN1_Output1.txt')
    	open(unit = 12, file = 'LaborFN1_Output2.txt')
    	open(unit = 13, file = 'LaborFN1_Output3.txt')
    	open(unit = 14, file = 'LaborFN1_Output4.txt')
    	open(unit = 15, file = 'LaborFN1_Output5.txt')
    	open(unit = 16, file = 'LaborFN1_Output6.txt')
    	            
    	open(unit = 21, file = 'LaborFN1_Output7.txt')
...
c     Gibbs sampler
	...
	do ir = 1, initer
          ...
          if (ir - ir/5*5 .eq. 1) then
              ...
              write(13,2) icount, delta, sigmas, v1
	...
	do ir = 1, niter
     ...
     +                           + theta(jj)/dfloat(niter1)
	        ...
	        mean_delta = mean_delta + delta/dfloat(niter1)
      
	        mean_sigmas = mean_sigmas + sigmas/dfloat(niter1)
      
	        mean_v1 = mean_v1 + v1/dfloat(niter1)
     ...
     +                                    /dfloat(niter1)
              ...
              bardic = bardic 
     +               + gen_dic(betaa, sigmay, theta, delta, 
     +                         sigmas, v1, phi, bmu, omega)
     +                 /dfloat(niter1)
...
c     Posterior estimates      
    ...
    3 format(100f20.10)   
      ...
      real*8 function gen_dic(betaa, sigmay, theta, delta, 
     +                        sigmas, v1, phi, bmu, omega)
      implicit real*8 (a-h, o-z)
	integer, parameter :: ni = 446, nj = 15, nb = 4
	integer, parameter :: nc = 3, nu = 4, nt = 7
	...
	real*8 api, omegai(nb,nb)
      real*8 sij, ystar, yystar		
	real*8 si, xxt(nt), xt, xp
      real*8 xxp(np), bstar(nb)	
      real*8 suma, sumb, bAb
	real*8 ddelta, ssigma, vv1

      real*8 bound, errabs, errrel, errest, cdf
      integer ipvt(nb)
	...
	real*8 pdf1, pdf2, pdf3, pdf4, dic	
      ...
      common /vyystar/yystar
      common /vddelta/ddelta
      common /vssigma/ssigma
      common /vvv1/vv1
	...
	det = det1*10.d0**det2
     ...
     +             - ystar**2/(2.d0*sigmay)
	    ...
	    errabs = 0.d0 ; errrel = 0.00001d0
	    bound = 0.d0 ; interv = 1
	    call dqdagi(df,bound,interv,errabs,errrel,cdf,errest)
	...
	gen_dic = dic
      ...
      real*8 function df(zeta)
      implicit real*8 (a-h, o-z)
	integer, parameter :: ni = 446, nj = 15, nb = 4
	integer, parameter :: nc = 3, nu = 4, nt = 7
	integer, parameter :: nx = 2, np = 5, nl = 3

	real*8 ezeta, delta, sigmas, v1
	real*8 yystar, ddelta, ssigma, vv1
      
      real*8 zeta, api, ystar, temp
	...
	delta = ddelta
      ...
      ystar = yystar - delta*(zeta - ezeta)

      temp = -dlog(api*sigmas)/2.d0 
     +     + dlngam((v1+1.d0)/2.d0) - dlngam(v1/2.d0) 
     +     - (v1+1.d0)/2.d0*dlog(1.d0 + ystar**2/sigmas)

      df = dexp(temp)
     +     *dexp(-zeta**2/2.d0)*dsqrt(2.d0/api)
      
      end function	

	include 'LaborFN1_Gibbs.f'
	include 'optim1.f'
	include 'gilks2.f'
	include 'tnorm.f'
